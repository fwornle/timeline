...
export function useTimelineData(baseUrl: string) {
  ...
  // Use the API base URL from config, not the repository URL
  const gitService = new GitService(initialConfig.apiBaseUrl, baseUrl || initialConfig.repositories.git, {
    maxAttempts: 3,
    initialDelay: 1000,
    maxDelay: 10000,
    timeout: 15000,
  ...
  const specService = new SpecStoryService(initialConfig.apiBaseUrl, baseUrl || initialConfig.repositories.spec, {
    maxAttempts: 3,
    initialDelay: 1000,
    maxDelay: 10000,
    timeout: 15000,
  ...
  const fetchTimelineData = useCallback(async (
    ...
    // Validate the repository URL before proceeding
    ...
    if (!forceRefresh && !sourceToRetry && hasValidRepoData(baseUrl)) {
      logger.info('data', 'Using cached data, skipping network fetch');
      const cachedData = loadRepoData(baseUrl);
      if (cachedData) {
        // Process cached data and update state
        const gitEvents = cachedData.gitEvents.map(event => ({
          ...event,
          timestamp: new Date(event.timestamp)
        }));
    ...
    try {
      ...
      if (baseUrl && !configUtils.isValidGitUrl(baseUrl)) {
      ...
      // Save data to cache if we have successful results
      if (baseUrl && (gitEvents.length > 0 || specEvents.length > 0)) {
        saveRepoData(baseUrl, gitEvents, specEvents);
        logger.info('data', 'Saved repository data to cache', {
          baseUrl,
  ...
  }, [filter, gitService, specService, state.events, baseUrl, hasValidRepoData, loadRepoData]);
