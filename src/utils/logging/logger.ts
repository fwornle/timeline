 ...
 * Logger.ts
 *
 * TypeScript Logger class for the timeline application.
...
export class Logger {
    ...
    try {
      ...
      if (storedValue) {
        ...
        if (Array.isArray(parsedValue)) {
          console.log(`[Logger] Loaded ${key} from localStorage:`, parsedValue);
          return new Set(parsedValue);
        }
        console.warn(`[Logger] Invalid format for ${key} in localStorage. Using default.`);
      }
    } catch (e) {
      console.error(`[Logger] Error loading ${key} from localStorage:`, e);
    }
    console.log(`[Logger] Using default ${key}:`, defaultValue);
  ...
  static setActiveLevels(levelsIterable: Iterable<string>): void {
    ...
    console.log('%c[Logger] Active levels set to:', 'color: blue; font-weight: bold;', [...Logger.activeLevels]);
  }
  ...
  static setActiveCategories(categoriesIterable: Iterable<string>): void {
    ...
    console.log('%c[Logger] Active categories set to:', 'color: blue; font-weight: bold;', [...Logger.activeCategories]);
  }
  ...
  static enableCategory(categoryName: string): void {
    ...
    if (!Logger.activeCategories.has(categoryName)) {
      ...
      Logger._saveSetting('activeLogCategories', Logger.activeCategories);
      console.log('%c[Logger] Enabled category:', 'color: blue; font-weight: bold;', categoryName);
    }
  }
  ...
  static disableCategory(categoryName: string): void {
    if (Logger.activeCategories.delete(categoryName)) {
      Logger._saveSetting('activeLogCategories', Logger.activeCategories);
      console.log('%c[Logger] Disabled category:', 'color: blue; font-weight: bold;', categoryName);
    }
  }
}
  ...
  log(level: any, component: string, topic: string, message: string, data?: Record<string, any>) {
    ...
    if (typeof level === 'number') {
      switch (level) {
        ...
        case 3: levelStr = Logger.Levels.ERROR; break;
        default: levelStr = Logger.Levels.INFO; break;
      }
    } else if (typeof level === 'string') {
      levelStr = level;
    }

    Logger.log(levelStr, component, `[${topic}] ${message}`, data);
  }
}
