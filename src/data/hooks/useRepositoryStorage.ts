...
export function useRepositoryStorage() {
  ...
  const [lastRepoUrl, setLastRepoUrl] = useState<string>(() => {
    try {
      ...
      Logger.error(Logger.Categories.CACHE, 'Failed to load last repository URL', { error });
      return '';
    }
  });
  ...
  const saveRepoUrl = useCallback((url: string) => {
    try {
      ...
      setLastRepoUrl(url);
      Logger.info(Logger.Categories.CACHE, 'Saved repository URL', { url });
    } catch (error) {
      Logger.error(Logger.Categories.CACHE, 'Failed to save repository URL', { error, url });
    }
  }, []);
  ...
  const saveRepoData = useCallback((url: string, gitEvents: unknown[], specEvents: unknown[], isMocked: boolean = false) => {
    ...
    try {
      ...
      Logger.info(Logger.Categories.CACHE, 'Saved repository data', {
        url,
      ...
      Logger.error(Logger.Categories.CACHE, 'Failed to save repository data', { error, url });
    }
  }, []);
  ...
  const loadRepoData = useCallback((url: string): { gitEvents: unknown[], specEvents: unknown[], isMocked?: boolean } | null => {
    ...
    try {
      ...
      if (Date.now() - data.timestamp > CACHE_EXPIRATION) {
        Logger.info(Logger.Categories.CACHE, 'Repository data is expired', { url });
        localStorage.removeItem(`${REPO_DATA_PREFIX}${url}`);
        return null;
      }

      Logger.info(Logger.Categories.CACHE, 'Loaded repository data from cache', {
        url,
      ...
      Logger.error(Logger.Categories.CACHE, 'Failed to load repository data', { error, url });
      return null;
    }
  }, []);
  ...
  const clearRepoData = useCallback((url: string) => {
    ...
    try {
      localStorage.removeItem(`${REPO_DATA_PREFIX}${url}`);
      Logger.info(Logger.Categories.CACHE, 'Cleared repository data', { url });
    } catch (error) {
      Logger.error(Logger.Categories.CACHE, 'Failed to clear repository data', { error, url });
    }
  }, []);
  ...
  const purgeRepoData = useCallback((url: string) => {
    ...
    try {
      ...
      const dataStr = localStorage.getItem(`${REPO_DATA_PREFIX}${url}`);
      if (!dataStr) {
        Logger.info(Logger.Categories.CACHE, 'No data to purge for repository', { url });
        return;
      }
      ...
      Logger.info(Logger.Categories.CACHE, 'Purged repository data and marked as mocked', { url });
    } catch (error) {
      Logger.error(Logger.Categories.CACHE, 'Failed to purge repository data', { error, url });
  ...
  const hasValidRepoData = useCallback((url: string): boolean => {
    ...
    try {
      ...
      Logger.info(Logger.Categories.CACHE, 'Repository data cache status', {
        url,
        isValid,
        hasEvents,
        isMocked,
      ...
      Logger.error(Logger.Categories.CACHE, 'Error checking repository data validity', { error, url });
      return false;
    }
  }, []);
